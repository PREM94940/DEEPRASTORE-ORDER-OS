...
(async () => {
  ...
  await page.type('input[placeholder="e.g. 9876543210"]', '9998887711');
  const testName = 'Test Live Validation 11';
  await page.type('input[placeholder="e.g. Priya Sharma"]', testName);
  await page.type('input[placeholder="e.g. priya@example.com"]', 'testlive11@example.com');
  ...
  await page.type('input[placeholder="e.g. DP-1205"]', 'LP-011');
  await page.type('input[placeholder="e.g. 1042"]', '1500'); 
  await page.type('input[placeholder="e.g. UTR123456789"]', 'LIVEUTR111111');
  await page.type('textarea[placeholder*="style references"]', 'Live Test Note 11');
  
  fs.writeFileSync('dummy11.png', 'dummy file');
  const fileInputs = await page.$$('input[type="file"]');
  if (fileInputs.length > 0) {
      await fileInputs[0].uploadFile('dummy11.png');
  }
  ...
  await page.screenshot({ path: 'validation-live-1-customer-11.png' });
  ...
  await page.screenshot({ path: 'validation-live-2-intake-11.png' });
  ...
  if (!found) {
    ...
    await page.screenshot({ path: 'validation-live-3-approved-11.png' });
